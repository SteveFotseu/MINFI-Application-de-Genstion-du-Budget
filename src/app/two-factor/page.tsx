...
// RÔLE     : Vérification OTP — connexions suivantes (firstLogin=false)
// Corps envoyé : { email, code, mfaToken }
...
export default function TwoFactorPage() {
  ...
  useEffect(() => {
    ...
    if (!email || !token) { router.replace(APP_ROUTES.LOGIN); return; }
    setUserEmail(email);
    setMfaToken(token);
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await verifyTwoFactor({ email: userEmail, code: otpValue, mfaToken });
      ...
      if (response.accessToken) {
        ...
        if (uc) {
          saveUserContext({
            id:        uc.userId,      // ✅ "userId" (pas "id")
            firstName: uc.firstName,
            lastName:  uc.lastName,
            email:     uc.email,
            role:      uc.role,        // ✅ "role" est bien là
            mandats:   uc.mandats ?? [],
          });
        }
  ...
  return (
    ...
    >
      {errorMsg && (
        <div className="alert alert--error" role="alert"><IconAlert /> {errorMsg}</div>
      )}
      {successMsg && (
        <div className="alert alert--success" role="status"><IconCheck /> {successMsg}</div>
      )}
      {userEmail && (
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            ...
            <div className="otp-container" role="group">
              {otp.map((digit, i) => (
                <input
                  key={i}
