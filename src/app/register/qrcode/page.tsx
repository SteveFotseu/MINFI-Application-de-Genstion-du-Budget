...
//
// RÔLE     : Page de configuration MFA — PREMIÈRE CONNEXION UNIQUEMENT
//
// FLUX :
//   1. Affiche le QR code (secretImageUri stocké en sessionStorage)
//   2. L'utilisateur le scanne avec Google/Microsoft Authenticator
//   3. L'utilisateur saisit le code TOTP à 6 chiffres
//   4. POST /api/v1/auth/setup-mfa { email, code, mfaToken }
//   5. ✅ Code correct → accessToken + userContext reçus
//      → Token sauvegardé → redirection dashboard selon rôle
//   6. ❌ Code incorrect → message d'erreur, nouvelle saisie
// ============================================================
...
export default function QrCodePage() {
  ...
  useEffect(() => {
    ...
    const storedMfaToken = sessionStorage.getItem('gbe_mfa_token');

    if (!storedQr || !storedEmail) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // POST /api/v1/auth/setup-mfa { email, code, mfaToken }
      ...
      if (response.accessToken) {
        // ✅ Sauvegarder le token et le contexte utilisateur
        ...
        setSuccessMsg('Authentification configurée avec succès ! Redirection…');
      ...
      setHasError(true);
      setErrorMsg(err instanceof ApiError ? err.message : 'Code invalide ou expiré. Réessayez.');
      setOtp(Array(TWO_FACTOR_CODE_LENGTH).fill(''));
      refs.current[0]?.focus();
    } finally {
      setIsLoading(false);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--clr-off-white)', fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 20px' }}>
        <div style={{ width: '100%', maxWidth: 520, background: 'var(--clr-white)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-card)', overflow: 'hidden', animation: 'cardReveal .45s cubic-bezier(.22,.68,0,1.2) both' }}>
          ...
          <div style={{ padding: '32px' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.4rem', fontWeight: 700, color: 'var(--clr-navy)', marginBottom: 6 }}>
                Configuration de la double authentification
              </h1>
              <p style={{ fontSize: '.85rem', color: 'var(--clr-gray-400)', lineHeight: 1.6 }}>
                Cette étape est <strong style={{ color: 'var(--clr-navy)' }}>unique</strong> — vous ne la verrez plus lors des prochaines connexions.
              </p>
            </div>

            {/* ÉTAPE 1 : QR code */}
            <div style={{ background: 'var(--clr-gray-50)', borderRadius: 'var(--radius-md)', padding: '16px', marginBottom: 20, border: '1px solid var(--clr-gray-100)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <span style={{ width: 24, height: 24, background: 'var(--clr-navy)', color: '#fff', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.75rem', fontWeight: 700, flexShrink: 0 }}>1</span>
                <p style={{ fontWeight: 600, fontSize: '.875rem', color: 'var(--clr-navy)' }}>
                  Scannez ce QR code avec votre application d&apos;authentification
                </p>
              </div>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}>
                <div style={{ padding: 12, background: '#fff', borderRadius: 10, border: '2px solid var(--clr-gray-100)', boxShadow: '0 2px 12px rgba(0,0,0,.06)' }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={qrCodeUrl} alt="QR Code 2FA" style={{ width: 180, height: 180, display: 'block' }} />
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div style={{ background: 'var(--clr-gray-50)', borderRadius: 'var(--radius-md)', padding: '16px', marginBottom: 16, border: '1px solid var(--clr-gray-100)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                  <span style={{ width: 24, height: 24, background: codeIsFull ? 'var(--clr-green)' : 'var(--clr-navy)', color: '#fff', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.75rem', fontWeight: 700, flexShrink: 0, transition: 'background .2s' }}>2</span>
                  <p style={{ fontWeight: 600, fontSize: '.875rem', color: 'var(--clr-navy)' }}>
                    Entrez le code à 6 chiffres affiché dans l&apos;application
                  </p>
                </div>

                {errorMsg && <div className="alert alert--error" role="alert" style={{ marginBottom: 12, fontSize: '.8rem' }}><IconAlert /> {errorMsg}</div>}
                {successMsg && <div className="alert alert--success" role="status" style={{ marginBottom: 12, fontSize: '.8rem' }}><IconCheck /> {successMsg}</div>}

                <div className="otp-container" role="group" style={{ marginBottom: 8 }}>
                  {otp.map((digit, i) => (
              ...
              <button
                ...
                style={{
                  ...
                  background: !codeIsFull || isLoading || !!successMsg ? 'var(--clr-gray-200)' : 'linear-gradient(135deg, var(--clr-navy) 0%, var(--clr-navy-mid) 100%)',
                  color: !codeIsFull || isLoading || !!successMsg ? 'var(--clr-gray-400)' : '#fff',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  transition: 'all .2s',
                }}
              >
                {isLoading ? (
                  <><span style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin .65s linear infinite' }} /> Vérification…</>
                ) : successMsg ? (
                  <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20,6 9,17 4,12"/></svg> Accès autorisé !</>
                ) : (
                  <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9,12 11,14 15,10"/></svg> Valider et accéder à mon espace</>
                )}
              </button>
            </form>

            {userEmail && <p style={{ textAlign: 'center', marginTop: 14, fontSize: '.75rem', color: 'var(--clr-gray-400)' }}>Compte : <strong style={{ color: 'var(--clr-gray-600)' }}>{userEmail}</strong></p>}
